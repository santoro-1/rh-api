...
- 内容规划与字幕虽通过同一公开接口聚合返回，但服务端必须在调用独立字幕请求前
  先提交音乐、视觉和标题分支。该中间状态不可缓存；如果字幕阶段中断，下次请求
  复用已成功的封面标题并只补齐字幕。
...
## 29. 请求风暴治理（2026-09-03）

本地 JYD 工作台对 `/api/auth/center/verify` 使用 5 秒进程内成功缓存、同 token singleflight、
请求内用户复用和独立 3 次/10 秒、打开 15 秒的连接失败保护。账号校验固定使用 8 秒上限，
并从异步认证中间件转交线程池执行，不能把阻塞式远程 I/O 直接放回事件循环。登录、退出、
账号交接、401 和进程关闭必须使相关缓存与进行中请求失效；日志不得记录 token 或 cookie。

网站历史任务页 `/tasks` 使用权限与日期过滤后的服务端分页，默认每页 20 条，只允许 20 或
50 条。路由仅检查当前页数字人缩略图；缺失、非法或不可访问的路径直接向模板提供不可用状态，
模板不得输出对应图片 API 地址。存在的图片使用 `loading=lazy`、固定尺寸以及
`Cache-Control: private, max-age=3600`。

活动任务由 `GET /api/tasks/statuses?ids=...` 批量返回当前页所需的最小状态，最多 50 个 ID，
普通用户的查询在数据库层按所有者过滤，未知或无权 ID 直接省略。响应使用 `no-store`。前端同一
时刻只允许一个状态请求，页面隐藏时暂停、返回前台立即更新，网络失败按 5、10、20、30 秒退避，
当前页无活动任务后停止。禁止恢复固定间隔的 `location.reload()` 整页刷新。

批次详情继续使用原有状态与内容修订局部刷新，但必须遵循同一套浏览器端保护：一个页面只保留
一个未完成轮询，页面隐藏时暂停并取消未完成请求，重新可见时立即恢复，网络失败按
5、10、20、30 秒退避，批次进入终态后停止。
