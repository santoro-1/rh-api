...
`/api/workbench/ltx-*` 合同。它不同于旧网页快速 LTX：所有音频（包括不超过 30 秒的音频）都先
...
- 数字人口播不超过 32.8 秒、LTX 音频不超过 30 秒：直接创建一个标准视频任务。
- 超过对应工作流上限：自动建立长音频项目。两者目标分段均约 30 秒；数字人硬上限
  32.8 秒，LTX 硬上限 30 秒。数字人另在临时模型输入端追加 2 秒静音，总输入不超过 35 秒；
...
当前迁移头为 `0037_multi_camera_web`。`0031` 新增双池基础实体、执行模式与
...
和分段机位快照；这些迁移均不重建既有父表。统一内容分析缓存从
...
- 字幕长度使用“有效字符”口径：每个汉字、阿拉伯数字和英文字母各计 1，标点、空格和换行
  ...
  算出的最少安全断点数量一致。
- 字幕质量校验失败会把具体原因反馈给同一专用 Prompt，最多重试 3 次；仍不合格时由服务端
  使用最少安全断点确定性兜底。无法安全拆开的超长数字、英文或词语返回独立失败，不伪造成功。
...
`usage=seam_broll`、`direct_concept_ids` 和 `segment_boundary_us`；Prompt v2 允许明确对象、同一
动作、同一具体场景或自然编辑型陪衬，拒绝仅宽泛同类或无关候选，并返回结构化匹配原因。
