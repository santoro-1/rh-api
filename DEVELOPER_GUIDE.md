...
- 数字人音频不超过 35 秒、LTX 音频不超过 30 秒：直接创建一个标准视频任务。
- 超过对应工作流上限：自动建立长音频项目。两者目标分段均约 30 秒；数字人硬上限
  35 秒，LTX 硬上限 30 秒。LTX 的较短上限用于降低高清参考视频的显存风险。
...
时间轴，系统按真实时间范围规划分段；数字人按约 30 秒、最长 35 秒切分，LTX 按最长
...
- 历史任务中可能仍冻结 `workbench_final_segment_tail_seconds=1`。该字段只为兼容旧输入
  ...
  时长，避免把不存在的尾部误判为视频缺失。
- 上传音频和历史非精确时间轴入口仍执行原有、最多 `0.5` 秒且不跨越 `45` 秒的安全尾垫；
  本次只撤销新版工作台的额外 1 秒规则。
