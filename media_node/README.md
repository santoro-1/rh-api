...
4. 编辑 `media_node\.env`：
   ...
   - 默认 `ASR_DEVICE=cpu`，不会占用其他 GPU ASR 的显存；Worker v4 会声明
     `h3_audio_alignment` 和 `h3_head_trim` 能力，分别领取 H3 生成前音频对齐和成片片头识别任务。
...
节点启动后，H3 生成前对齐由页面创建任务，服务器依次记录 `media.h3_asr_claimed`（领取）和
`media.h3_asr_completed`（回传）；生成后片头识别还会先记录 `video.h3_head_trim_queued`。
节点断线时任务保持
