...
> 文件名配对。当前数据库迁移头为 `0018_segment_video_merge`。
...
- 当前迁移头为 `0018_segment_video_merge`。每次发布必须先运行只读预检，以实际服务器
  版本、服务状态、迁移和健康检查为准。
...
- Windows 启动器已改用只读进程句柄判断 PID 是否存活，修复重复双击无法打开页面、
  点击停止却误报“没有运行”的问题。
- 当前新增数据库迁移头为 `0018_segment_video_merge`，部署或本地更新代码后必须
