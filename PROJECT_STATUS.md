...
当前处于 **“生产环境已上线，本地集中修改和回归验证中，尚未发布本轮改动”** 阶段。

- 生产站点已隔离部署到 `video.lanyingjk01.com`，项目目录为
...
- 本地新增独立“长音频拆分”入口和媒体 Worker。可插拔对齐接口默认改为
  `funasr_http`：独立 Paraformer 服务输出字词时间戳，主站将识别 token 与完整
  原脚本对齐后规划约 30 秒、最长 45 秒的切点；识别文本不会替换原脚本。用户可直接
  播放原始音频区间并人工调整，确认后才切割音视频并创建标准子任务。FunASR 服务
  代码、隔离依赖和真实项目只读基准脚本已加入；本机依赖/模型仍待用户安装并用
  259 秒真实音频验证，尚未部署到服务器。`heuristic` 只保留为显式降级 provider。
...
- Windows 启动器已改用只读进程句柄判断 PID 是否存活，修复重复双击无法打开页面、
  点击停止却误报“没有运行”的问题。
- 当前新增数据库迁移头为 `0012_long_audio_projects`，部署或本地更新代码后必须
...
| 左/右人物音频 | `739` / `738` | `audio` | 双人模式必填；单人模式不提交，保留 AI App 默认值 |
