...
> 当前数据库迁移头为 `0013_remote_media_worker`，完整测试为 `121 passed`。
...
- 独立“长音频拆分”入口支持 Windows 远程媒体节点。Paraformer 输出字词时间戳，
  节点将 token 与完整原脚本对齐后规划约 30 秒、最长 45 秒的切点；识别文本不会
  替换原脚本。用户试听并确认后，节点再串行切割音视频、回传结果，服务器校验后
  创建标准子任务。259 秒真实音频已在笔记本 CPU 暖机后约 24 秒完成 ASR。
...
- Windows 启动器已改用只读进程句柄判断 PID 是否存活，修复重复双击无法打开页面、
  点击停止却误报“没有运行”的问题。
- 当前新增数据库迁移头为 `0013_remote_media_worker`，部署或本地更新代码后必须
