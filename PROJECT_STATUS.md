...
> 2026-08-22 当前开发树已把新旧 LTX 入口的单段硬上限从 30 秒进一步收紧为 20 秒；新
> LTX 工作台优先按约 18 秒寻找 MiniMax 句间切点，其他 LTX 完整流程统一保证单段不超过
> 20 秒。数字人仍保持约 30 秒目标、32.8 秒口播硬上限和 2 秒临时静音尾。

> 2026-08-20 当前开发树已将独立 LTX 工作台固定接入逐段 SeedVR2 `plus/48G`：每个 LTX
> 成功分段先保存源片段，再只对该片段执行清晰化，清晰结果完成后才参与合并。失败重试只重跑
> 当前失败阶段；旧 LTX 网页不自动放大，老数字人工作台 SeedVR2 开关不变。独立本地工作台
> 正式启动复用 JYD 的线上服务器配置，本机 `8000` 只作为测试覆盖。尚未部署生产。
> 当前云端完整 mock 回归 `372 passed`，独立工作台 `9 passed`。
>
> 2026-08-19 当前开发树已将新旧入口的 LTX 对口型硬上限统一从 45 秒收紧为 30 秒：正好
> 30 秒仍作为单段，超过 30 秒进入 ASR/时间轴切分；数字人 35 秒规则不变。本次没有接入
> 当时尚未接入 SeedVR2；该描述已由上方 2026-08-20 决策覆盖。没有修改已经提交的远端任务，也尚未部署生产。

> 2026-08-19 当前开发树新增独立上传音频 LTX 工作台合同，迁移头为
> `0036_ltx_workbench_preparation`。新入口按每一行完整视频与完整音频比较时长，所有音频统一
> ASR 对齐表格原稿，提示词只由云端固定编译，不接 MiniMax、补帧或视频规格标准化；SeedVR2
> 现按上方 2026-08-20 决策只接独立 LTX 工作台。
> OOM 不自动重复付费。独立本地项目位于 `..\ltx_lip_sync_workbench`，可生成老剪映渲染器
> 接受的精确字幕/BGM 交接任务。当前改动未部署生产，全量 mock 回归 `370 passed`。

> 2026-08-14 当前开发树把旧网页单条、批量和长音频的 SeedVR2 当次开关改为默认关闭，
> 用户仍可按次开启；新版工作台 4A 继续强制开启。生产完整数据备份改为成功后自动轮换，
> 默认只保留最近 2 份，不触碰代码包、独立数据库快照或配置备份。修改尚未部署生产。

> 2026-08-12 历史开发树新增迁移头 `0033_generation_task_seedvr2_switch`。管理员可按用户设置
> 数字人 `default`（24G）或 `plus`（48G），新任务冻结该实例；当时旧网页开关默认开启，
> 现已由上方 2026-08-14 的默认关闭决策覆盖。新版工作台仍强制 SeedVR2，SeedVR2 本身仍固定
> `plus`（48G）。本条覆盖下方 2026-08-09/10 的固定 48G 历史快照。

> 2026-08-11 当前开发树已把数字人分段策略收紧为目标约 30 秒、硬上限 35 秒，覆盖
> MiniMax 句级时间戳、上传长音频和旧单条/批量入口；LTX 对口型继续保持 45 秒硬上限。
> 修改仍在本地，未取消生产任务、未部署、未推送。

> 2026-08-11 当前开发树已实现新版工作台管理员多 RunningHub 账号资源池：任务所有者与
> 真实执行账号分离，数字人和对应 SeedVR2 锁定同一账号，逐次尝试保存远程账号与 ID，
> 模糊提交禁止盲目切换。工作台 4A 已改为逐行持久化后台启动并在费用确认中提供账号复选框。
> 普通用户、旧网页、`legacy_web` 和 LTX 均保持原逻辑。云端全量 mock 回归为 `298 passed`；
> 工作台本需求相关测试通过，全量为 `377 passed, 4 unrelated failed`（用户现有语义素材变化和
> 旧免责声明断言）。当前仍是未部署本地开发状态。
...
- 统一生成页支持数字人和 LTX；数字人按管理员用户配置冻结 `default`（24G）或 `plus`
  （48G）。旧网页默认关闭、可按次开启 SeedVR2，新版工作台始终接续 SeedVR2；SeedVR2 固定 48G。
...
### RunningHub 成功分段的画质异常与单段重跑（待办，2026-08-08）

- 当前工作台 `composition/retry` 只重试状态为 `FAILED`、`DOWNLOAD_FAILED` 的
  RunningHub 任务，或状态为 `MERGE_FAILED` 的合并阶段。第三方任务已经返回成功、文件也
  下载成功，但画面存在曝光、肤色或色偏漂移时，目前不能指定该成功分段重新生成。
- 真实案例已确认是某个约 32 秒的 RunningHub 原始分段在前中段正常、接近结尾时逐渐发白
  偏青；相邻分段恢复正常。原始 MP4 的色彩元数据与其他分段一致，异常在拉取到的源画面
  中已经存在，不是浏览器、FFmpeg 合并或剪映额外调色造成。相同图片后续重新生成没有复现，
  暂按低概率生成模型时序漂移处理，不把特定图片直接判定为必现条件。
- 第一阶段建议增加“人工指定单段重跑”：必须再次确认 RunningHub 费用；保留旧成功任务和
  原始文件用于追溯，只新建被选分段的付费任务；成功后按原 `segment_index`、音频时间范围、
  当前图片 SHA-256 替换该分段的当前版本，并仅失效、重建基础视频及其下游剪映成片，不重做
  MiniMax 音频、时间戳或其他正常分段。
- 第二阶段可增加非付费画质质检：对每段开头、中间、结尾抽帧，结合段内亮度/色偏变化和
  相邻分段差异给出“疑似画质漂移”提示。初期只进入人工复核，不自动调用 RunningHub；待
  真实样本足够后再确定阈值，避免正常动作、背景光变化被误判并产生额外费用。
- 本条仅记录后续设计边界，尚未实现接口、数据库版本指针、页面按钮或自动质检，也未部署。

### H3 本地 ASR 与远程媒体节点边界调整（待确认，2026-08-26）

- 当前生产版本 `b951a0e` 在 `MEDIA_PROCESSING_MODE=remote` 下，把 H3 生成前音频对齐和
  生成后片头识别都持久化为 `h3_remote_asr_jobs`。Ubuntu 服务器不运行 FunASR；Windows
  媒体节点领取任务后，调用该节点本机 `127.0.0.1:18084` 的 ASR，再把结构化结果回传服务器。
- PublicVideo 剪映工作台本身也会启动或复用本机 `127.0.0.1:18084`，用于精确字幕。两者可以
  部署在同一台 Windows 电脑并复用同一个 ASR 服务，但仅启动工作台不等于媒体节点已经连接；
  当前仍需要独立媒体节点进程主动领取网站 H3 ASR 任务。
- 2026-08-26 生产核查确认：服务器代码、`0046` 迁移、四个 systemd 服务和健康接口均正常，
  但外部媒体节点在服务器更新后没有继续领取任务。这暴露出产品预期与运行边界不一致：用户
  容易认为“部署了工作台本地 ASR”就足以同时服务网站 H3，实际还依赖独立节点进程。
- 建议优先方案是保留服务器持久化任务、租约、幂等和审计合同，不把 ASR 端口暴露到公网；
  将 H3 媒体节点领取能力并入 PublicVideo 统一启动/退出生命周期，自动复用工作台本机 ASR。
  这样服务器仍不安装模型，操作人员也不再单独维护“启动媒体节点”窗口。预计属于中等偏小
  调整：主要涉及统一启动器、媒体 Worker 启停/日志、配置注入、升级包及跨项目测试，通常不需
  新增数据库迁移，也不应修改已有 `0045`/`0046`。
- 备选方案是让 Ubuntu 生产服务器重新安装并运行 FunASR，再把 H3 对齐切回服务器本地调用。
  该方案代码改动不大，但会扩大生产依赖、模型占用、部署与健康检查范围，并恢复此前已取消的
  服务器 ASR 运维负担；不建议作为默认方案。直接把 Windows 的 `18084` 暴露给服务器也不接受。
- 验收边界：只启动 `PublicVideoWorkbenchLauncher.exe` 后，网站 H3 音频对齐和片头识别能够
  自动被同机处理；启动器退出后任务保持 `PENDING` 或租约到期安全回队；工作台字幕继续使用
  同一 ASR；不得重复提交 RunningHub 付费任务，不得覆盖本地 `data`、账号、素材或历史任务。
- 本条仅记录现状、推荐方向和验收边界，尚未修改运行逻辑、打包脚本或生产配置。
...
- 新版工作台时间戳任务使用内部 `exact_timestamps` 标记，各段先将 RunningHub 结束时间向上
  取整到整秒；当前规则仅把整个任务最后一段的 `end_time` 再加 1 秒用于表情收尾，不修改或
  补静音到 MiniMax 音频。中间段不加，单段任务视为最后段；本地后处理保留该秒画面。
