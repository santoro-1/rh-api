...
BACKUP_KEEP_COUNT="${RUNNINGHUB_BACKUP_KEEP_COUNT:-1}"
...
rotate_code_backups() {
    local remove_count index filename
    local -a code_backups

    # Deployment rollback archives use a separate exact name. Keep the newest
    # one after a successful deploy; snapshots with any other name are left alone.
    ...
    for ((index = 0; index < remove_count; index++)); do
        filename="${code_backups[$index]}"
        if [[ ! "$filename" =~ ^runninghub-video-code-pre-[0-9a-f]{12}-[0-9]{8}T[0-9]{6}Z\.tar\.gz$ ]]; then
            echo "代码备份文件名不符合安全轮换规则，拒绝删除：$filename" >&2
            exit 1
        fi
        rm -f -- "$BACKUP_DIR/$filename"
        echo "已轮换旧代码备份：$BACKUP_DIR/$filename"
    done
}
