...
每个数字人分段成功后都会继续进入固定 `plus`（48G）的 SeedVR2 放大流；工作台默认下载
...
旧网页单片段和多片段也统一使用 SeedVR2 清晰结果；旧多片段仍只生成原有快速顺序拼接
预览，不套用新版工作台的标准化算法。
...
创建任务时先在统一页面选择数字人或视频对口型工作流。数字人固定使用 Plus 48G；视频对口型可为当前任务选择 `default` 或 `plus`。

数字人每个子片段包含两次一对一 RunningHub 生成：一次数字人、一次 SeedVR2 48G 清晰化。
