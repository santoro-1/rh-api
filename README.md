...
放大阶段再从本次 SeedVR2 快照独立原子选号。普通用户、旧网页、`legacy_web` 和 LTX 保持原逻辑。
...
旧网页单条、批量和长音频数字人入口提供默认开启的 SeedVR2 放大开关。开关值在任务创建时
冻结；关闭后数字人源片段下载成功即作为最终结果，不创建 SeedVR2 任务。旧多片段仍只生成
...
启用放大时，数字人每个子片段包含两次一对一 RunningHub 生成：一次数字人、一次 SeedVR2 48G 清晰化。
例如 6 个子片段在正常无重试时产生 6 个数字人任务和 6 个 SeedVR2 任务；页面仍显示
...
数字人远端成功后父任务继续保持活动，并在页面/工作台显示 `VIDEO_ENHANCING`；只有
...
- “完整流程”：网页可直接填写，也可导入只含“任务编号、口播脚本”两列的
  ...
  返回的官方句级时间戳切成约 30 秒的可见子任务；数字人最长 35 秒，视频对口型最长
  45 秒，不再根据文字长度
...
直接上传音频时，用户仍按一行一组上传完整素材。系统逐行检查音频：数字人不超过
35 秒、视频对口型不超过 45 秒时直接创建一个视频任务，超过对应上限则在同一个批次
行下自动拆成多个子任务：
...
两条路径都生成约 30 秒的分段；数字人硬上限 35 秒，降低后续 SeedVR2 长片段超时风险，
视频对口型继续保持 45 秒硬上限。“长音频拆分后先试听确认”默认关闭并
...
- generation_task_enhancements：每个数字人子任务一对一的 SeedVR2 状态、源片段、清晰
