...
新版工作台的 4A 音频来自 MiniMax 原始时间戳。普通分段提交 RunningHub 时只把实际小数
时长向上取整为整秒；整个任务的最后一段（单段任务即该段）在这个整秒结束时间上再加
`1` 秒，让数字人停止说话后完成表情收尾。该秒只扩展 RunningHub `end_time`，不修改、补静音
或重新上传 MiniMax 音频；4A 合并和工作台时间轴会保留这 1 秒画面。
...
- “完整流程”：网页可直接填写，也可导入只含“任务编号、口播脚本”两列的
  ...
  返回的官方句级时间戳切成可见子任务；数字人最长 35 秒，视频对口型最长
  30 秒，不再根据文字长度
...
35 秒、视频对口型不超过 30 秒时直接创建一个视频任务，超过对应上限则在同一个批次
行下自动拆成多个子任务：
...
数字人路径生成约 30 秒、硬上限 35 秒的分段；视频对口型目标与硬上限均为 30 秒，
