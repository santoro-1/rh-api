...
新版工作台管理员可在每次 4A 费用确认中从其获授权的 RunningHub 执行账号资源池勾选本次
账号，默认重新全选；普通用户、旧网页和 `legacy_web` 任务继续使用业务所有者自己的单账号。
每个数字人子任务按真实执行账号独立占用容量，并把同一账号从数字人阶段锁定到对应
SeedVR2 完成；任务、批次和结果归属始终是登录管理员。远程 ID、查询、取消、下载和恢复按
逐次尝试绑定原账号，提交结果不明确时保留容量并禁止跨账号盲目重提。API Key 只在云端
加密保存，工作台只接收和提交内部账号 ID。
