# RunningHub 视频生成中转站

这是一个 FastAPI + SQLite + Jinja2 的单服务器应用：浏览器仅访问本站；Web 进程创建任务；独立 Worker 使用服务端保存且加密的 RunningHub API Key 上传素材、提交工作流、轮询状态并把视频下载到服务器。当前支持数字人视频和 LTX 2.3 视频对口型两个工作流。

## 文档导航

设备授权开发文档已更新至 v1.20：正式服务器签名根已在受控目录初始化，只导回公钥并编入 JYD；正式根下 JYD/Pu-LTX 全量分别为 1206/61 项通过。首次完整包在全新解压启动时发现并修复 PowerShell UTF-8 BOM 兼容问题；修正版无素材完整包已通过最终 ZIP 递归扫描、清单比对、双工作台实际启动、正式公钥诊断和 ASR 导入检查。新增服务器受控模式切换工具，以及本机 A、250 B 的权威双机证据记录/复制拒绝/四构建更新校验工具；专项 `6 passed`，加入工具后的当前 RunningHub 完整工作树为 `888 passed`。生产 `ENFORCE` 未启用，设备授权数据库未修改，业务代码未部署；真实硬件/跨机和连续升级仍待验收，不能据此宣称整个产品已经完成防转拷。当前交接见第 25.6 节；正常更新不重复激活的规则见第 1、8、11 节。

- [DEVELOPER_GUIDE.md](DEVELOPER_GUIDE.md)：当前架构、业务流程、本地开发、状态机、
  测试、媒体节点、生产发布和扩展方式的统一开发入口。
- [CHANGELOG.md](CHANGELOG.md)：面向版本的功能变化、兼容性说明和验证记录。
- [数字人网站与剪映工作台集成说明.md](数字人网站与剪映工作台集成说明.md)：数字人账号、任务分流、剪映工作台集成、日志规划和本地验收记录。
- [工作台请求风暴与页面卡顿修复开发文档.md](工作台请求风暴与页面卡顿修复开发文档.md)：账号校验缓存与并发合并、登录中心失败保护、历史任务分页、缩略图按需加载和状态局部轮询的实施与验收方案；本期不做物理电脑识别。
- [工作台设备授权与免重复激活开发文档.md](工作台设备授权与免重复激活开发文档.md)：设备审批、持钥验证、更新免激活、打包检查和验收计划；包含授权基础、H3/普通数字人/声音付费准入、本地核心与恢复、独立 Agent、中断任务人工核实和签名根工具。LTX、正式公钥初始化/编入及双机连续升级验收仍未完成，不可作为防转拷完成版发布。
- [MAINTENANCE.md](MAINTENANCE.md)：模块边界、迁移红线、日志规范和修改检查清单。
- [deploy/README.md](deploy/README.md)：生产预检、发布、验收、备份和恢复。
- [media_node/README.md](media_node/README.md)：固定电脑媒体节点的安装、迁移及完整包/
  小更新包分发。
- [WORKFLOW_EXTENSION.md](WORKFLOW_EXTENSION.md)：新增 RunningHub 工作流适配器的最小步骤。
- [SeedVR2视频清晰化流程开发文档.md](SeedVR2视频清晰化流程开发文档.md)：数字人新旧入口统一接入
  SeedVR2 分段放大、固定 48G、状态机、费用保护及剪映工作台兼容的实施与验收依据。
- [多机位图片单音频数字人流程开发文档.md](多机位图片单音频数字人流程开发文档.md)：老网站受控
  多机位测试入口、20 秒自然停顿分段、机位排列快照、权限和有序片段交付合同。
- [PROJECT_STATUS.md](PROJECT_STATUS.md)：阶段快照和历史交接；当前技术事实以开发者指南
  和代码为准。
- [../语音标点停顿配方开发文档.md](../语音标点停顿配方开发文档.md)：工作台脚本标点、
  人工停顿、MiniMax 编译、幂等与费用保护的跨项目开发依据（当前仅完成方案，尚未实施）。
- [语义视觉素材库与智能编排开发文档.md](语义视觉素材库与智能编排开发文档.md)：单次大模型
  内容分析、字幕边界复用、图片/视频统一调度、相关素材与空镜的实施与验收依据；阶段 0～7
  已完成，B-roll 源素材质量作为后续素材库迭代。
- [语义视觉素材统一协议设计.md](语义视觉素材统一协议设计.md)：语义图片和视频的 catalog、
  asset、recipe、路径及 renderer 底层协议。
- [语义视觉素材打标规范_v2.md](语义视觉素材打标规范_v2.md)：素材语义角色、自动触发边界、
  抽象概念防误下钻、空镜用途门槛，以及现有正式库重新打标和迁移的依据。
- [语义视觉素材v2.1交叉审查报告_20260813.md](语义视觉素材v2.1交叉审查报告_20260813.md)：
  210 素材审计结论、catalog v3 兼容层、迁移回滚安全门、测试证据与尚需人工确认的上线门槛。

## 本地前置条件

- 已安装 Anaconda Python（当前项目按你的 base 环境验证）
- Python 3.11+（需求目标为 3.12，代码兼容）
- 已安装 ffmpeg，并可在 PATH 中调用 `ffmpeg` 和 `ffprobe`

## 初始化

在项目根目录执行：

    conda activate base
    python -m pip install -r requirements.txt
    Copy-Item .env.example .env
    python -m alembic upgrade head
    python -m scripts.create_admin admin

首次创建管理员时会提示输入密码。RunningHub、MiniMax 与豆包 Ark 的账号连接、工作流 ID
和用户级参数在登录后的用户管理页配置；豆包内容分析的服务端统一并发上限通过
`ARK_MAX_CONCURRENCY` 配置，默认且硬上限为 10；开启
`ARK_REQUEST_MANAGER_ENABLED` 后，内容与视觉分析共享专用公平队列、540 秒总预算和持久
幂等账本。API Key 与工作流访问密码不会
回显，且只以 Fernet 加密密文保存在 SQLite 中。LTX 发布设置未开启“加密访问”时，
工作流访问密码保持为空即可。语速、音量、语调等每批使用参数不放在用户
配置中，而是在批量生成页选择。

MiniMax H3 的用户开通状态也在用户管理页配置，并且独立于数字人“一控多/双池”运行模式。
RunningHub 执行账号页只维护真实账号的 API Key、并发、启停、健康和余额；三个工作流各自在
系统工作流配置页统一维护自己的 Workflow ID / AI App ID、访问密码、实例和默认提示词。网站
用户可使用哪些 RunningHub 账号也在用户管理页分配；未分配用户不会继承其他人的池账号。

开通后的用户在 `/generate/batch` 素材类型中可选择“MiniMax H3 多参考生成”。H3 第一版不在
该页面生成语音：用户上传整批共用的 `0～4` 张人物图，并按行上传参考视频、已准备好的成品
音频和一致原稿。JYD 正式入口在本机复用或生成 FunASR 字词对齐，并随请求提交
`jyd.h3-safe-cut-alignment.v1`；服务端严格校验冻结原稿摘要、完整音频摘要和 MiniMax 声音版本后
规划 4～15 秒分段，不依赖生产服务器的 `127.0.0.1` ASR。未提交该合同的旧入口仍保留原兼容
回退。“计算分段与费用”不会创建 RunningHub 付费任务，只有再次明确确认费用才进入 H3 队列。
老网页 H3 区域还提供批次级“人工总体提示词”完整覆盖：留空时继续使用系统六段式 Prompt；
填写后每个自动分段原样使用该文本，系统不再自动注入分段台词、音频复用、人物一致性或首尾
锚点约束。覆盖文本、版本和摘要会在费用预检时冻结，后续确认、重试和 soft-chain 解锁均复用
同一快照。

.env 至少应修改：

- APP_SECRET_KEY：一段随机长字符串
- 生产环境必须设置 APP_ENCRYPTION_KEY：Fernet URL-safe Base64 key
- COOKIE_SECURE=false：仅限本地 HTTP；上线 HTTPS 后改为 true
- MAX_IMAGE_SIZE_MB：数字人 JPG/PNG/WEBP 单张图片上传上限，默认 200 MB
- MAX_VIDEO_SIZE_MB：LTX 源视频上传上限，默认 500 MB
- LONG_AUDIO_ALIGNMENT_PROVIDER：长音频默认对齐器，当前为 `funasr_http`
- ASR_BASE_URL：独立 ASR 服务地址，本地默认 `http://127.0.0.1:18084`
- MEDIA_PROCESSING_MODE：`local` 为同机处理，`remote` 为授权电脑主动领任务
- MEDIA_WORKER_TOKEN：远程模式专用随机令牌，生产环境至少 32 个字符

可用以下方式生成加密 Key：

    python -c "from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())"

## 启动

首次初始化完成后，双击项目根目录的 `启动系统.cmd` 即可。它会在后台完成迁移，
同时启动 Web、语音 Worker、媒体 Worker 和视频 Worker；如果 `.asr-runtime`
中的独立环境已经安装完成，也会启动本地 ASR 服务。随后自动打开默认批量生成页。
无需保留 PowerShell 窗口；重复双击不会重复启动同一套服务。

剪映工作台可以直接使用本网站现有账号登录，并通过 `/api/workbench/tasks` 拉取当前账号自己的旧任务收件箱记录。批次详情中的“后处理清单”是给人查看的中文页面；对应 `/api/batches/.../postproduction` 仍保留为机器接口。旧收件箱继续按原规则提示多片段人工处理；新版 `/app/new` 的 4A 画面模块按 2026-08-04 决策自动拼接多片段并保存为基础视频，同时保留全部原始片段。上传粗剪替换入口留待模块 5。

新版工作台模块 3 还通过 `/api/workbench/voices*`、`/api/workbench/voice-creations*`
和 `/api/workbench/audio-batches*` 复用本网站的 MiniMax 官方/自定义音色、声音制作任务
和异步音频 Worker。工作台音频批次固定开启审核，生成完成后停在
`AWAITING_REVIEW`，供工作台同步 MP3 和原始时间戳；本阶段不会自动进入 RunningHub
画面生成。浏览器仍只访问工作台后端，MiniMax Key 不下发。

工作台声音生成只检查 MiniMax 配置、当前账号音色、脚本、语音参数和费用确认，不要求配置
旧版独立 RunningHub Key、分配视频执行账号或启用数字人工作流。视频配置尚未准备好时也能先
生成、试听和下载声音；RunningHub 账号与工作流校验仍在原有视频生成链路执行。

工作台提交声音时退出或丢失回执，可通过 `/api/workbench/audio-batches/lookup` 按原请求键只读查询
当前用户自己的声音批次及输入摘要，不新建任务、不调用供应商。工作台核对输入一致后接回原任务；
查询无结果不代表原请求未计费，不应自动重新提交。此恢复能力应先更新本网站，再更新工作台，无数据库迁移。

模块 4A 已增加工作台专用的画面启动、状态、失败阶段重试和基础视频下载接口。用户在
工作台确认费用后，既有音频审核门才放行到原有音频/视频 Worker；单片段和多片段结果
都按已确认音频时长标准化，多片段按顺序拼接，并在接缝使用 0.25 秒保时长叠化。
新版工作台每个数字人分段成功后都会继续进入固定 `plus`（48G）的 SeedVR2 放大流；工作台默认下载
放大后的清晰片段，数字人源片段仍在云端保留并可单独鉴权下载。标准化结果只标记为
`base_video`。音频完成后、4A 启动前，工作台仍可替换图片；启动请求绑定最后一次选择的当前图片。

新版工作台 RunningHub 资源池有两个冻结分支。`RUNNINGHUB_DUAL_POOL_ENABLED=false`（默认）
继续使用 `same_account_v1`：管理员每次 4A 勾选数字人账号，同一账号顺序完成数字人与
SeedVR2。开关开启且当前用户具有 ID 级授权时使用 `dual_pool_v1`：费用确认分别展示数字人池
和 SeedVR2 池，两组每次重新默认全选且都至少保留一个；数字人源片段落盘后释放数字人槽位，
放大阶段再从本次 SeedVR2 快照独立原子选号。旧网页单次、批量和长音频数字人任务统一冻结
用户管理页当时分配的 RunningHub 执行账号集合，不再读取用户历史专属 Key；需要重新提交的旧
失败任务在人工重试时首次冻结当前分配集合。独立 LTX 工作台仍使用用户现有 RunningHub 配置，
不进入数字人资源池。

执行模式与两组账号 ID 都作为不可变操作快照保存，开关或授权后续变化不会迁移已启动任务。
业务归属始终是登录用户；远程 ID、查询、取消、下载、恢复和看门狗始终使用对应尝试的原执行
账号。SeedVR2 普通失败留在原账号重试，只有 401/403、无权限、Key/App/工作流不存在等明确
账号不可用证据才允许下一次付费尝试在本次快照内换健康账号；下载失败和提交结果不明确均不
换号重提。API Key 只在云端加密保存，工作台只接收和提交内部账号 ID。

费用确认会明确显示“一控多/双池”和本次候选账号；任务启动后，工作台按分段显示实际执行
账号。一控多显示同一个账号完成数字人与 SeedVR2，双池分别显示两个阶段账号，尚未原子预留
的阶段显示“待分配”。云端只返回内部 ID、备注名称和阶段状态，不返回任何凭据或工作流配置。
账号清单还会返回 `accountStatus` 的安全余额摘要，费用确认显示剩余 RH 币和查询时间。同一真实
API Key 的缓存按不可逆指纹共享；管理员可在两个资源池页面手动刷新。H3 每次打开执行账号选择
前会强制实时读取余额，余额为 0、读取失败或未返回余额的账号保留显示但不可勾选；Worker 在每次
上传素材和创建远程任务前再次读取，发现余额为 0 时不消耗自动重试次数，并改用本批冻结快照内
其他有余额账号。完整 Key、请求 Body 和第三方完整响应仍不会进入浏览器或日志。

新旧入口通过批次字段 `source_channel` 明确隔离：历史数据及原网页创建的批次为
`legacy_web`，新工作台创建的声音批次为 `new_workbench`。原网页批次列表只显示
`legacy_web`；工作台的单片段标准化和多片段按音频时长补帧/裁切不会应用到旧网页。
旧网页单条、批量和长音频数字人入口提供默认关闭、可按次开启的 SeedVR2 放大开关。开关值在
任务创建时冻结；关闭后数字人源片段下载成功即作为最终结果，不创建 SeedVR2 任务。旧多片段仍只生成
原有快速顺序拼接预览，不套用新版工作台的标准化算法。

数字人口播按约 30 秒、硬上限 32.8 秒切分。每个新生成分段在上传 RunningHub 前临时追加
2 秒静音，原始 MiniMax 音频、raw cues 和字幕时间轴保持不变，模型输入总时长不超过 35 秒。
新版工作台自动拼接时裁掉所有中间分段的生成尾，只保留最后一段的 2 秒收尾画面；旧入口
继续交付独立原始片段，不增加自动正式拼接。

模块 4B 位于工作台本地后端，使用其现有剪映渲染队列为 `base_video` 添加 MiniMax
时间轴单行字幕和可选 BGM，再登记 `composition_video`。本项目没有为 4B 新增后端、
账号、RunningHub 调用或变体任务；4B 失败重试不会重新调用本项目已成功的付费任务。

后续智能内容分析模块 1～8 已完成本地 mock 自动化闭环。本项目的
`POST /api/workbench/content-analysis` 每次只接收一条精确脚本，并可接收不含路径和时间的
本地视觉候选；一次 Ark 请求返回分别校验的 `music_intent`、无时间戳 `subtitle_units`、
三字段 `visual_plan` 和唯一两行 `title`。工作台仍负责本地选材、MiniMax `raw_cues` 映射、
时间调度、剪映写入、模型标题到封面的映射、正文固定单行标题、
真实字体宽度排版和本地音乐唯一 Top1。跨项目测试会把本项目实际响应直接交给
工作台消费。内容分析失败不得触发 MiniMax、RunningHub 或剪映，也不得使已有基础视频失效。
工作台请求可携带 `analysis_operation_id` 和相对剩余预算；服务器通过专用队列、短期进行中
合并及 `ark_analysis_operations` 账本防止断线重试形成第二份付费调用。`/healthz/ark` 只返回
队列、时延和熔断诊断，不包含脚本、API Key 或完整服务商响应。
Prompt v16 要求唯一两行封面标题独立满足平台安全和医疗科普约束：低风险体重管理用语从源头
改成自然中性表达，硬风险内容使用中性提示，不允许用谐音字、近形字、emoji 或符号伪装敏感词。
真实服务与生产发布仍需另行授权。

需要停止时双击 `停止系统.cmd`。管理员登录后可从“运行状态”页面查看四个服务的
心跳、CPU、内存、磁盘、FFmpeg、语音/媒体/视频队列数量和最近日志。日志位于
`data/logs/`，默认保留 7 天且单文件最大 10 MB，敏感凭证写入前会脱敏。

开发时仍可分别运行以下进程，以便在终端看即时输出：

    python -m scripts.serve_web
    python -m app.workers.audio_worker
    python -m app.workers.media_worker
    python -m app.workers.task_worker

修改 Worker 代码后需要重新启动对应 Worker；Web 进程不会替它重载。

## 固定电脑远程媒体节点

长音频的 ASR、切音频和切视频，以及 H3 生成前音频对齐和生成后片头 ASR 定位，可以放在 Windows 电脑执行，服务器只负责保存
任务、素材和最终子任务。节点使用 HTTPS 主动轮询服务器，因此不需要公网 IP、
端口映射、防火墙放行或把本机 ASR 暴露到公网。

处理顺序如下：

1. 用户仍在网站上传长音频、原脚本和源视频；H3 页面上传成品音频后，点击费用计算会先建立
   `h3_audio_alignment` 任务。
2. 服务器保存项目，固定电脑节点用独立令牌领取分析任务。
3. 对口型任务调用节点内的 FunASR；数字人任务只做 FFmpeg 静音检测。
4. 开启人工确认时用户先在网站试听；默认自动进入下一步。
5. 节点再次领取切割任务，使用本机 FFmpeg 串行切音频和所需视频。
6. 节点把严格命名的分段 ZIP 回传；服务器复核段数和时长后创建 RunningHub
   子任务。
7. H3 输入对齐由节点调用本机 FunASR 并回传带音频/原稿哈希绑定的切点，服务器收到后才建立费用快照。
8. H3 成片下载后当前直接保留完整片头并继续 soft-chain 解锁；保留的 `h3_head_trim` 能力只有在
   `H3_HEAD_TRIM_ENABLED` 重新开启后才会创建远程任务并由节点回传首词裁剪决定。

任务租约默认 30 分钟，节点每 60 秒续租。电脑断网或退出后，租约过期的任务会
自动回到队列，另一个节点可以继续领取。服务器与电脑使用同一个
`MEDIA_WORKER_TOKEN`，网站用户密码、RunningHub Key 和 MiniMax Key 都不会下发
到电脑。

节点首次配置：

```powershell
powershell -ExecutionPolicy Bypass -File .\media_node\install-media-node.ps1
notepad .\media_node\.env
```

把服务器脚本输出的 `MEDIA_WORKER_TOKEN` 填入 `media_node/.env`，确认
`MEDIA_WORKER_SERVER_URL=https://video.lanyingjk01.com`，然后双击
`media_node/启动媒体节点.cmd`。启动器会复用已经运行的兼容 ASR；若没有运行，则只在
`127.0.0.1:18084` 启动一个。关闭窗口会停止由该启动器创建的 ASR 和 Worker。
完整安装、迁移和端口冲突处理见 [`media_node/README.md`](media_node/README.md)。

节点工作目录默认是 `media_node/data`，每个任务成功或失败后都会清理。
第一版 ASR 与 FFmpeg 都限制为单任务串行，避免和电脑上已有的高精度 ASR、分镜
FFmpeg 同时抢占大量资源。

创建任务时先在统一页面选择数字人或视频对口型工作流。管理员可按用户把数字人实例配置为
`default`（24G）或 `plus`（48G）；配置只影响之后创建的任务并冻结到任务输入。视频对口型
继续使用自己的实例配置；SeedVR2 一旦启用仍固定使用 `plus`（48G）。

启用放大时，每个源生成子片段包含两次一对一 RunningHub 生成：一次数字人或 LTX、一次 SeedVR2 48G 清晰化。
例如 6 个子片段在正常无重试时产生 6 个源生成任务和 6 个 SeedVR2 任务；页面仍显示
6 个子片段。哪个数字人片段先完成，哪个立即进入自己的放大阶段，不等待整批完成。

任务统一先保存为 `PENDING`。Worker 按用户的最大同时任务数占用运行槽位并按创建时间依次提交；例如并发数为 2、一次创建 8 个任务时，2 个运行、6 个留在本地队列，任一槽位释放后自动补充下一个。

RunningHub 接受任务后，本站每 5 秒同步 `QUEUED`、`RUNNING`、`SUCCESS` 或 `FAILED`
状态，不再用一小时本地计时抢先判失败。为避免第三方状态永久卡死，远程任务从提交起
超过 4 小时仍未结束时进入安全看门狗：先读取一次最新状态，再调用 RunningHub 取消；
只有取消成功才释放本地槽位并标记失败，取消失败时保留原任务继续查询和重试取消。
源生成远端成功后父任务继续保持活动，并在页面/工作台显示 `VIDEO_ENHANCING`；只有
SeedVR2 视频下载完成后子片段才进入 `SUCCESS`，多片段合并也只读取清晰结果。

独立 LTX 工作台固定逐段执行 `LTX -> SeedVR2 plus/48G -> 清晰分段合并`，不提供开关；
旧 LTX 网页仍保持原行为，不自动接 SeedVR2。两类入口通过 `source_channel` 隔离。

## 批量生成

登录后的默认页面是“生成视频”。快速创建与 Excel/CSV 表格导入保持并列一级入口，
单次上传一个素材、批量上传多个素材；旧单次生成路由保留用于兼容。每次最多创建
50 条任务。“快速创建”不要求填写表格：先按任务顺序上传图片或视频，
直接上传音频模式再按相同顺序上传音频，系统按页面显示的第 1、2、3……项配对。
上传后可以拖动、上下移动或移除文件，创建前必须确认最终顺序。

另有独立的 `/generate/multi-camera` 受控测试页，仅授权表中的本地/服务器 `admin`、
`Cx_ceshi` 对应用户 ID 可见和可调用。它允许批量音频绑定包含 `1～N` 张有序图片的机位组：
不足 20 秒保持单段并只用机位 1，超过 20 秒优先按自然停顿做全局均衡切分；两机位交替，
三机位按 `123、132、213、231、321、312` 展开，四机位以上按字典序惰性展开全排列。
每个结果仍是现有的一张图片加一段音频的标准 `digital_human` 任务，RunningHub 适配器和
Worker 不变。该来源固定为 `multi_camera_web_v1`，不进入旧批次列表、不自动拼接，也不接入
剪映工作台；页面提供服务端预检、范围/连续批量映射、分段重试、逐段下载和整批有序 ZIP。

- 数字人当前只开放单人模式；双人模式保留底层适配能力，暂不对用户开放。
- 视频对口型的 Stand/Plus 按批次统一设置。
- 数字人直接上传音频时每行填写提示词；视频对口型每行只填写一次口播脚本。
- 视频对口型正向提示词只写“什么人、使用什么语言、音频中的完整台词”，不需要
  动作、镜头或画面描述。
- 建议文件名带 `01、02、03` 序号。系统以页面最终显示顺序为准，文件名只辅助用户核对。
- 同一张图片或同一段视频可以对应多条不同音频/脚本。素材只需上传一次，点击列表中
  的“复用”可增加一行引用；其他分组已有多条素材时可点击“补齐”，高级导入可点击
  “填满”。后台按素材 ID 而不是文件名绑定，因此同名文件不会混淆，也不会为复用的
  大视频重复占用暂存空间。
- “Excel / CSV 表格导入”直接显示。数字人模板只有“任务编号、提示词”两列，
  视频对口型和语音生成模板只有“任务编号、口播脚本”两列。
- 表格不填写素材文件名。图片、视频、总参考音频、左人物音频和右人物音频在网页
  分组上传，各组第 1、2、3……项与表格第 1、2、3……行对应。
- 任一组素材数量与表格行数不一致，或任一行参数无效时，整批只返回校验错误，
  不创建任何任务。
- 校验通过后，每行创建一条独立 `PENDING` 任务，并按清单顺序进入原有 FIFO 队列。
- 生成页顶部提供常显的“任务名称”。上传音频时默认取首个音频文件名（去掉扩展名），
  文案生成时默认取首个任务编号或画面文件名；多条任务附带数量，用户可在提交前修改。
  任务记录页优先显示该名称，批次 UUID 继续保留用于精确排障。
- 批次详情和历史列表提供汇总进度、失败项重试和手动删除。终态批次以及没有活动
  视频/语音 Worker 的本地卡住批次可以删除；仍在排队或远程运行的任务禁止删除。
- 只有“输入文案生成语音，并且最终只形成一个视频子任务”会在视频成功后进入自动
  BGM/字幕分支。MiniMax 返回的原始字幕时间戳会保留在后处理清单中。
- 上传音频或语音切出多个视频片段时，生成结果进入人工处理。视频 Worker
  生成的拼接文件只作为片段顺序检查预览，不作为正式成片；原始分段始终全部保留。

页面提供两种长期并存的音频入口：

- “上传现成音频”：按页面顺序与图片或视频直接配对，不依赖任何语音服务。
- “完整流程”：网页可直接填写，也可导入只含“任务编号、口播脚本”两列的
  Excel/CSV。每行脚本只提交一次 MiniMax 异步长文本语音任务；完成后按 MiniMax
  返回的官方句级时间戳切成可见子任务；数字人口播最长 32.8 秒，视频对口型最长
  20 秒，不再根据文字长度
  猜测切点。两种入口在“音频准备完成”后使用相同的本地视频队列。

脚本生成语音当前只支持数字人单人模式和视频对口型，不为双人数字人自动生成两路
音频。声音克隆和声音融合位于独立“声音管理”页面；只有试听后明确保存的音色和已
导入的官方系统音色才会出现在批量生成页。完整流程每批选择一个可用音色，并在页面
选择语速、音量、语调等参数。官方系统音色不收取 ¥9.9 克隆音色费，但文本合成仍按
MiniMax 账单计费；克隆、融合或设计音色首次正式使用可能另收音色费。长期音色同时
绑定站内用户和稳定的 MiniMax 官方
账号标识；同一官方账号轮换 API Key 不影响音色，管理员明确切换官方账号后旧账号
音色会被隔离。原始声音样本默认只保留 48 小时。

完整流程的数字人分段复用同一张图片。视频对口型会按音频分段的实际时间区间顺序
切割源视频，因此源视频时长必须不短于整段生成音频；每段正向提示词由可修改的
“人物 + 语言”前缀和该段原始脚本文字组成，不加入动作、镜头或画面描述。第一版
保留全部分段视频，不自动拼接；源视频不足时会保留整段音频，替换视频后可继续切分，
无需重新调用 MiniMax。

完整流程的“脚本语音设置”支持可选的批次级读音标注，输入 MiniMax 官方
`pronunciation_dict.tone` JSON 数组，例如：

```json
["燕少飞/(yan4)(shao3)(fei1)", "omg/oh my god"]
```

规则应用于本批次全部脚本；Excel/CSV 脚本清单仍只有“任务编号、口播脚本”两列。
批量页可通过“预览全部文案”集中查看完整脚本和最终视频提示词。

完整流程默认在语音生成完成后自动切分并进入视频队列。需要人工把关音色或读音时，
可在创建批次前勾选“语音生成后先审核”：系统会先暂停在完整音频，支持在线试听、
单条通过、重新生成和整批通过。重新生成会再次产生 MiniMax 文本合成费用，旧版本
不会被覆盖，可在批次详情中展开试听；只有通过的版本才会继续切分并创建 RunningHub
视频任务。

直接上传音频时，用户仍按一行一组上传完整素材。系统逐行检查音频：数字人口播不超过
32.8 秒、视频对口型不超过 20 秒时直接创建一个视频任务，超过对应上限则在同一个批次
行下自动拆成多个子任务：

- 视频对口型上传源视频、长音频和完整原脚本。默认 `funasr_http` provider 由独立
  Paraformer 服务返回字词时间戳，主站与原脚本对齐，在自然停顿切音频和源视频，
  并把每段原脚本写入对应子任务提示词。ASR 文本不会替换原脚本；运行实例默认
  Plus 48G。
- 数字人图生视频上传源图片和长音频，不调用 ASR，只用静音检测寻找自然停顿；
  系统切分音频后为所有子任务复用同一张图片和动作提示词。

数字人路径生成约 30 秒、口播硬上限 32.8 秒的分段，并在模型输入端追加 2 秒静音；视频对口型目标与硬上限均为 20 秒，
用于降低高清参考视频进入 LTX 时的显存风险。“长音频拆分后先试听确认”默认关闭并
自动继续创建 `generation_segments` 子任务；开启时才停在试听页调整边界。独立环境
安装与迁移方法见 `media_node/README.md`。

批量暂存素材默认最多合计 5 GB、保留 24 小时，可通过
`MAX_BATCH_ITEMS`、`MAX_BATCH_TOTAL_UPLOAD_MB` 和
`STAGED_ASSET_RETENTION_HOURS` 调整。定时清理命令会同时清理过期暂存素材。

任务详情页会轮询单个运行中任务；任务历史页在存在活动任务时每 5 秒自动刷新。
批次详情只在后台更新状态，不会整页刷新或重置滚动位置。管理员运行页面每 5 秒
更新资源和增量追加启动、领取、第三方提交/返回、成功与异常事件，不会整页刷新或打断当前
滚动位置；成功的批次状态轮询、日志轮询和健康检查会被隐藏，服务器上的原始日志
文件仍完整保留。

RunningHub 已明确返回 `FAILED` 的视频任务默认自动重试 3 次，分别等待 60、120、
240 秒后复用原素材重新排队；每次远程任务 ID 和失败详情都会保留。三次耗尽后，
仍可在任务历史页点击“失败重试”，人工重试会重新开始一轮最多 3 次的自动恢复，
并可能再次产生 RunningHub 费用。网络提交结果不明确时不会自动重提。仅下载失败会
保留远程 Task ID，只重试本地下载。次数与基础等待可通过
`RUNNINGHUB_AUTO_RETRY_LIMIT`、`RUNNINGHUB_AUTO_RETRY_BASE_DELAY_SECONDS`
调整。远程异常滞留看门狗默认 4 小时，可通过
`RUNNINGHUB_REMOTE_WATCHDOG_SECONDS` 调整；旧的
`RUNNINGHUB_TASK_TIMEOUT_SECONDS` 已停止使用。所有终态任务都可以删除，删除会同时清除任务记录、上传素材和本地结果，
且无法恢复；活动任务不能删除。
数字人和 SeedVR2 分别计数：数字人失败只重试数字人；SeedVR2 失败复用已保存的源片段，
只重试清晰化；SeedVR2 下载失败复用同一个远端 ID，只重新下载。提交结果不明确时两个
阶段都禁止自动付费重提。

任务历史支持按北京时间的开始日期和结束日期检索。每条任务都可以勾选，也可以
全选当前筛选结果并批量删除；批量删除只接受全部处于终态的任务，如果选择中包含
排队中或运行中的任务，本次操作不会删除任何记录。开始勾选后，列表自动刷新会暂缓，
避免清空当前选择。

## H3 多参考生成输出合同

H3 如果明确报远端 `LoadImage` 缺少 PNG，人工重试会在可精确确认原图的情况下生成画面不变的
临时上传副本，尝试取得新的远端文件名。无法确认素材、原图损坏或仍返回失效文件名时，会在
生成提交前停止并提示，不自动发起新的付费生成。原始图片、提示词、已成功分段不变；该恢复
不适用于 JPEG/WebP 或视频。每轮上传保留不含凭据的文件指纹回执，便于后续定位。

H3 Ref2VA 把图片、参考视频和 MiniMax 分段音频作为生成条件，节点 387 返回的是模型共同生成的
视频轨与 H3 解码音频轨。节点 138 的输入音频不能在下载后覆盖节点 387 的音轨，否则会改变
语音起点并破坏模型口型同步。

云端当前输出合同为 `h3.output.generated-av-head-trim-disabled.v4`：仍以 H3 返回的音画为同一个
权威同步对，并完整保留每个分段开头。`H3_HEAD_TRIM_ENABLED` 当前为 `False`，所以生产远程媒体
模式不会创建 `h3_head_trim` 任务，本地模式也不会为成片片头调用 FunASR；标准化仅做
stream-copy 重封装，`soft_chain` 从完整结果末帧抽取连续性图片。生成前用于安全切分上传音频的
`h3_audio_alignment` 仍正常运行，不受该开关影响。

原有首字前保留 40ms、ASR 异常时固定裁切 300ms，以及音视频同步裁切的实现和验证仍保留在开关
后，后续需要时可重新启用。重新启用时必须再次升级输出合同，避免完整保留版与裁切版互相复用。

输出合同版本已经进入内容摘要，旧的 v3 裁切、静音或换音轨结果不会被新任务误复用。H3
本地工作台继续负责合并和 `h3.jyd_handoff.v2` 交接；MiniMax 输入 raw cues 只作切段和来源审计。
工作台批次状态中的成功分段同时返回 `normalized_video_sha256` 和 `completed_at`。下载 URL 在同一
分段主动重生成前后保持稳定，因此 JYD 必须使用这两个字段识别结果版本，不能只按 URL 缓存。

工作台 H3 另有默认关闭的 `H3_DIRECT_DELIVERY_ENABLED` 灰度开关。开关开启且片头裁剪仍关闭时，
Worker 只把 RunningHub 返回的 HTTPS 下载地址、供应商任务号和稳定的 `result_signature` 写入
`output_metadata`，不再把 MP4 保存到云端磁盘；`soft_chain` 只从远端视频抽取一张末帧 PNG。
H3 状态接口通过 `video_delivery.mode=runninghub_direct` 把地址交给新版 JYD，由 JYD 直接下载到
本机缓存并计算本地 SHA-256。旧版 JYD 仍可访问原分段 `/video` 接口，该接口仅作无凭据转发兼容，
不会重新落盘。若以后启用片头裁剪，Worker 自动回到原有下载和后处理链路。

新建 H3 批次默认使用 `loop_anchor` 首尾同图模式：至少选择 1 张人物图，用户排序第一张固定为
`<Picture 1>`，同时约束每个自动分段的第一帧和最后一帧；该模式不会在第一张图前插入参考视频
抽帧，各段仍可并行。`fast` 和 `soft_chain` 继续作为可选模式。H3 请求生成余量默认从 0.5 秒
降为 0.1 秒，历史批次继续使用自身冻结值。

每行上传的参考视频在“计算分段与费用”阶段自动按 `0～3、3～6……` 切成无音轨动作片段，
最后不足 3 秒的尾段仍完整保留。系统使用 `balanced_bag_v1` 确定性均衡分配到该行的音频分段，
相邻分段在存在多个候选时不会使用同一片段；实际片段路径和 SHA-256 写入分段冻结快照，失败
重试、人工重生成和 `soft_chain` 下游激活均不重新排列。旧批次没有动作片段快照时继续使用原始
参考视频。

## 日常命令

    python -m pytest -q
    python -m alembic current
    python -m scripts.cleanup_files

清理命令仅处理终态任务：上传素材从任务结束时间起保留 48 小时，生成视频保留
7 天，可通过 `.env` 中的 `UPLOAD_RETENTION_DAYS` 和 `OUTPUT_RETENTION_DAYS`
调整。失败重试依赖原上传素材；超过保留期被清理后，需要重新创建任务并上传素材。

## 手动真实 RunningHub 联调

自动化测试完全 mock RunningHub，不会产生费用。仅在需要手动验证时，于当前 PowerShell 临时设置环境变量（不要写回代码或 .env.example）：

    $env:RUNNINGHUB_API_KEY = "你的新 Key"
    python runninghub_local_test.py --image .\sample.jpg --audio .\sample.mp3 --end 0:15
    Remove-Item Env:RUNNINGHUB_API_KEY

该命令会产生真实 RunningHub 消耗。网站联调则是在管理员页面把 Key 粘贴进对应用户配置后，登录该用户创建任务；Worker 会接手后续流程。

## 数据与安全

- SQLite 启用 WAL、外键与 busy timeout，适合一个 Web、一个语音 Worker、一个
  媒体调度 Worker 和一个视频 Worker 并发访问。
- 数据文件位于 data/uploads/用户ID/任务ID/ 和 data/outputs/用户ID/任务ID/。
- 用户密码使用 PBKDF2-SHA256 哈希；RunningHub Key 使用 Fernet 对称加密。
- 任务、图片预览和视频下载均进行登录与归属校验；管理员可查看全部任务。
- 所有登录、退出、配置修改和任务创建请求均校验 CSRF Token。
- 生产环境强制要求 HTTPS Cookie、随机应用密钥、稳定的 Fernet Key 和明确的可信域名。
- 请立即在 RunningHub 后台轮换旧 Notebook 中曾使用过的 API Key。

## 数据库表

- users：网站账号、密码哈希、管理员与启用状态
- runninghub_configs：一对一的加密 API Key 与该用户的 RunningHub 配置
- minimax_configs：一对一的 MiniMax 加密 API Key、稳定官方账号绑定、凭证指纹与调用节流配置；
  完全相同凭据的不同网站账号通过指纹共享自定义音色
- ark_configs：一对一的豆包 Ark 加密 API Key、模型、启用状态、超时与有限重试配置
- content_analysis_caches：按用户、脚本哈希、模型、契约和 Prompt 版本隔离的音乐/字幕/视觉/标题分析缓存
- minimax_voice_assets：该网站账号的临时声音样本和本地音色副本；同 Key 用户的自定义副本
  指向同一个 MiniMax provider voice ID，删除操作彼此隔离
- audio_generation_tasks：完整脚本、已保存音色、异步 MiniMax 任务编号、句级时间轴、
  语音参数、对齐方式、审核状态和当前生成版本
- audio_generation_attempts：每次 MiniMax 生成的完整音频、字幕、远程编号和审核结果
- workflow_configs：每个用户、每个工作流的远程 ID、实例类型、默认提示词和启用状态
- generation_tasks：本地任务 ID、远程 taskId、输入素材、时间范围、数字人实例与 SeedVR2
  开关快照、状态、错误、usage 与本地结果路径
- generation_task_enhancements：每个启用清晰化的数字人或独立 LTX 子任务一对一的 SeedVR2 状态、源片段、清晰
  片段、执行账号、远端 ID、哈希、usage 和独立重试时间
- generation_task_enhancement_attempts：SeedVR2 每次提交尝试的不可变账号、远端 ID、
  固定 48G 载荷摘要和终态审计
- generation_batches / generation_batch_items：带 `source_channel`、`correlation_id` 的批次元数据、脚本
  父任务和原始清单行
- generation_segments：完整流程的分段脚本、时间区间、分段素材和 RunningHub 子任务关系
- multi_camera_user_access / multi_camera_batch_configs：多机位测试的用户 ID 授权与批次策略快照
- multi_camera_image_groups / multi_camera_image_group_assets：批次内有序机位组及图片哈希快照
- multi_camera_item_bindings / multi_camera_segment_bindings：音频到图片组、分段到具体机位的冻结映射
- staged_assets：批量创建前已经校验、等待清单引用的暂存素材
- long_audio_projects：长音频原始素材、脚本、可编辑分段草稿、远程节点租约、
  资源指标、处理状态和目标批次

## 生产部署

仓库已提供 Ubuntu 单服务器部署模板，包括：

- Nginx 反向代理和大文件上传配置；
- Web、语音 Worker、媒体 Worker、视频 Worker、自动备份和自动清理的 systemd 服务；
- 生产环境变量模板、预检、备份与恢复脚本；
- HTTPS、上线验收、更新和日志检查步骤。

完整说明见 [deploy/README.md](deploy/README.md)。正式部署时使用一个 Web 进程、
一个语音 Worker、一个媒体 Worker 和一个视频 Worker；公网不直接开放 8000 端口。
服务器默认不再安装或启动 FunASR；长媒体使用 Windows 节点时，服务器媒体 Worker
只负责保留队列心跳，实际 ASR 与 FFmpeg 由授权节点执行。

## 工作台语义视觉图片与视频

云端保留独立 `POST /api/workbench/visual-analysis`。工作台只提交精确脚本、字符候选和
允许的概念；云端使用用户级 Ark 配置做语境消歧，并按用户、脚本、目录、候选集合、模型、
Prompt 和契约版本独立缓存。接口不会接收本地素材路径或音视频时间，失败也不会改变声音、
字幕、BGM、RunningHub 或已有视频。请求契约为 `jyd.visual-analysis.request.v1`，响应契约为
`jyd.visual-analysis.v1`。

新统一链路通过 `/api/workbench/content-analysis` 的可选 `visual_context` 在同一次 Ark 调用中
产生 `visual_plan`；工作台项目主流程已经完成阶段 2 切换，原视觉按钮也复用统一协调器。
visual context 输入锚点包含短语上下文和 `explicit/enrichment/seam_broll` 用途，但模型输出仍
严格只有 `anchor_id/concept_id/priority`；自动空镜只接受脚本直接命中的同一具体对象、动作或
明确场景并返回 priority 2。priority 1、编辑型氛围、同类宽相关、勉强相关或无关候选都不会
自动进入配方。数字人真实分段在首轮
分析后才生成，因此工作台 4B 会自动用独立视觉接口补一次仅含 `seam_broll` 的轻量分析；该调用
按候选摘要幂等，只合并接缝结果，失败或没有合格素材时保留原溶解并继续生成。
统一视觉锚点使用 `VA` 前缀，与字幕安全边界的 `B` 前缀隔离；旧工作台的 `B` 视觉输入会先
规范化。模型偶发返回未提供或越权的单项时只丢弃该项，其他合法画面继续使用。
