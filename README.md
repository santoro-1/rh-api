...
长音频的 ASR、切音频和切视频，以及 H3 成片的片头 ASR 定位，可以放在 Windows 电脑执行，服务器只负责保存
...
6. 节点把严格命名的分段 ZIP 回传；服务器复核段数和时长后创建 RunningHub
   子任务。
7. H3 成片下载后，服务器另建 `h3_head_trim` 任务；节点只回传首词裁剪决定，服务器继续完成
   同步裁剪和 soft-chain 解锁。
...
持久化为 `h3_head_trim_jobs` 并由固定电脑主动领取，生产服务器不再尝试访问自身的
