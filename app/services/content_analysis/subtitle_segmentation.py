...
SUBTITLE_ANALYSIS_PROMPT_VERSION = "jyd.subtitle-analysis.prompt.v20"
...
def subtitle_system_prompt() -> str:
    ...
    return dedent(
        ...
        4. 在满足每段不超过10个有效字符的前提下，新增逗号数量必须最少。
        5. 10个有效字符是上限，不是建议长度，不得为了形成更短字幕而继续拆分。
        6. 只有在“已经确定必须切分”的情况下，才根据语义选择断点位置；语义完整性只决定在哪里切，不能决定是否切。
        7. 优先把每段切到接近10个有效字符，避免产生不必要的2～5字碎片。
        8. 不得拆开完整词语、专有名词、人物名称、品牌名称、数字与单位、固定搭配、紧密短语。
        9. 不得让“的、地、得、了、着、过、吗、呢、啊、呀”等助词或语气词单独成为一段。
        10. 原文已有标点不得修改、删除或移动，不得在已有标点前后重复添加逗号。
        11. 除新增必要的中文逗号以外，原文必须逐字保持一致，包括数字、空格、换行和标点。

        【优先级】
        原文完全不变 ＞ 每段不超过10个有效字符 ＞ 新增逗号数量最少 ＞ 语义自然。
        ...
        第四步：逐一检查所有新增逗号。在保证所有字幕段均不超过10个有效字符的前提下，删除所有不必要的新增逗号，确保新增逗号数量最少。
...
def validate_subtitle_split(
    ...
    *,
    ...
    for start, end in _natural_spans(original_script):
        ...
        if len(span_insertions) != len(minimum_plan):
            return SubtitleValidationResult(
                False,
                (
