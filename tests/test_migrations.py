...
def test_h3_remote_asr_migration_preserves_existing_head_trim_job() -> None:
    ...
    try:
        ...
        connection.close()
        assert revision == "0048_legacy_task_runninghub_pool"
        assert migrated[:5] == (
            1,
            "h3-task",
            "h3_head_trim",
...
def test_legacy_task_pool_migration_adds_nullable_candidate_snapshot() -> None:
    runtime = PROJECT_ROOT / "tests" / ".runtime"
    runtime.mkdir(parents=True, exist_ok=True)
    database = runtime / f"migration-legacy-task-pool-{uuid.uuid4().hex}.db"
    try:
        _run_alembic(database, "0047_h3_prompt_override")
        _run_alembic(database, "head")
        with sqlite3.connect(database) as connection:
            revision = connection.execute(
                "SELECT version_num FROM alembic_version"
        ...
        connection.close()
        assert "runninghub_execution_account_ids_json" not in downgraded_columns
    finally:
        database.unlink(missing_ok=True)
...
def test_system_voice_category_migration_resumes_after_interrupted_add_column():
    ...
    try:
        ...
        assert version == "0048_legacy_task_runninghub_pool"
